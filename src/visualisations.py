...
def create_visualisation(final_corpus):
    ...
    key1, value1 = PoS_analysis(INTERMEDIATE_DIR + '/no_stop_sentences1.txt')
    key2, value2 = PoS_analysis(INTERMEDIATE_DIR + '/no_stop_sentences2.txt')
    barplot_PoS(key1, value1, value2)

    mood_complex = verbal_features(typev='Mood',
                                   filename=INTERMEDIATE_DIR + '/no_stop_sentences1.txt')
    mood_simple = verbal_features(typev='Mood',
                                  filename=INTERMEDIATE_DIR + '/no_stop_sentences2.txt')
