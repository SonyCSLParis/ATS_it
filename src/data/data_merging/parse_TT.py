...
def write_on_file(output_data, originali, semplici):
    with open(INTERMEDIATE_DIR + output_data, 'w') as outfile:
        writer = csv.writer(outfile)
        header = ['Normal', 'Simple']
        writer.writerow(header)
        for i in range(len(originali)):
            if semplici[i] != '':
                writer.writerow([originali[i], semplici[i]])
                            ...
                            for line in infile:
                                if not 'semplificato' in line and not 'originale' in line and not 'doc id' in line and not '</doc>' in line:
                                    ...
                                    if 'frase_al' in line:
                                        ...
                                        original_s.append((id, frase_all, frase))
                                        ...
                                        semplici_s.append((id, frase))

                            for i in range(len(original_s)):
                                if original_s[i][0] == original_s[i][1] and original_s[i][1] != '':
                                    for ele in semplici_s:
                                        if ele[0] == original_s[i][0]:
                                            original_ter.append(original_s[i][2])
                                            semplici_ter.append(ele[1])

                                else:
                                    for ele in semplici_s:
                                        if ele[0] == original_s[i][1]:
                                            original_ter.append(original_s[i][2])
                                            semplici_ter.append(ele[1])



write_on_file('/teacher.csv', original_teach, semplici_teach)
write_on_file('/terence.csv', original_ter, semplici_ter)
