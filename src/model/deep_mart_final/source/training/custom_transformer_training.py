...
from src.model.deep_mart_final.source.analysis.plotting import Plotter
from src.model.deep_mart_final.source.custom_transformer.model_implementation import transformer_model_utils as tu
from src.model.deep_mart_final.source.custom_transformer.model_implementation.transformer import get_model
from src.model.deep_mart_final.source.preprocessing.dataset import TextDataset
...
class TransformerTrainer:
    ...
    def start_training(
        ...
        #iterators è un metodo implementato nella classe TextDataset, itera tra train e test set
        data_iter = dataset.iterators()[0]

        #qui inizia il training effettivo
        ...
        for epoch in range(arguments.epochs):
            ...
            if (
                ...
                os.mkdir(f"{save_model_path}/model_checkpoint_epoch-{epoch}.pt")
        ...
        if arguments.save_model_path is not None:
            torch.save(model.state_dict(), f"{save_model_path}/model2.pt")
            TransformerTrainer.__logger.info(
                f"Model saved to: {save_model_path}/model2.pt."
            )
        ...
        if arguments.loss_plot_save_path is not None:
            Plotter.plot_loss(
                losses,
                f"{arguments.loss_plot_save_path}/loss_plot2.html",
                x_label="Epoch",
            )
